...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("elasticnet_ablation.log", encoding='utf-8'),
...
def create_feature_subsets(feature_cols):
    ...
    return {
        ...
        'RDKit+ Fingerprint': no_mordred_chemberta,
        'RDKit_Only': rdkit_only
    }

def optimize_elasticnet_model(X_train, y_train, feature_cols):
    """优化ElasticNet模型（简化参数搜索）"""
    # ElasticNet参数优化
    enet_pipe = Pipeline([
    ...
    enet_params = {
        ...
        'enet__max_iter': [5000, 10000],
        'enet__tol': [1e-4, 1e-3]
    }
    
    logger.info("开始优化ElasticNet模型...")
    enet_search = RandomizedSearchCV(
        enet_pipe, enet_params, n_iter=20, cv=3,  # 减少迭代次数和交叉验证折数
        scoring='r2', n_jobs=-1, verbose=1, random_state=42
    )
    enet_search.fit(X_train, y_train)
    
    logger.info(f"ElasticNet最佳参数: {enet_search.best_params_}")
    logger.info(f"ElasticNet最佳交叉验证R2: {enet_search.best_score_:.4f}")
    
    return enet_search.best_estimator_, enet_search
...
def train_elasticnet_model(X_train, X_test, y_train, y_test, feature_subset, subset_name):
    """训练ElasticNet模型"""
    ...
    # 修复：传递正确的参数
    enet_model, enet_search = optimize_elasticnet_model(X_train_subset, y_train, feature_subset)
    
    # 评估模型
    y_pred = enet_model.predict(X_test_subset)
    metrics = evaluate_model(y_test, y_pred)
    
    logger.info(f"{subset_name} 模型训练完成，测试集R²: {metrics['R2']:.4f}, MRE: {metrics['MRE']:.2f}%")
...
def plot_ablation_radar_chart(ablation_results, output_dir):
    ...
    for i, (model_name, result) in enumerate(ablation_results.items()):
        ...
        for metric in metrics:
            if metric == 'R²':
                ...
                # 误差指标越小越好，需要反转
                if metric == 'MAE':
                    max_val = max([r['metrics']['MAE'] for r in ablation_results.values()]) * 1.1
                    normalized_values.append(1 - (metrics_values['MAE'] / max_val))
                elif metric == 'MSE':
                    max_val = max([r['metrics']['MSE'] for r in ablation_results.values()]) * 1.1
                    normalized_values.append(1 - (metrics_values['MSE'] / max_val))
                elif metric == 'RMSE':
                    max_val = max([r['metrics']['RMSE'] for r in ablation_results.values()]) * 1.1
                    normalized_values.append(1 - (metrics_values['RMSE'] / max_val))
                elif metric == 'MRE':
                    max_val = max([r['metrics']['MRE'] for r in ablation_results.values()]) * 1.1
                    normalized_values.append(1 - (metrics_values['MRE'] / max_val))
    ...
    # 添加标题 - 字体调小
    plt.title('ElasticNet消融实验性能比较', fontsize=10, fontweight='bold', pad=20)
...
def main():
    try:
        ...
        for subset_name, subset_features in feature_subsets.items():
            result = train_elasticnet_model(X_train, X_test, y_train, y_test, subset_features, subset_name)
            ablation_results[subset_name] = result
        
        # 5. 绘制雷达图
        logger.info("绘制消融实验雷达图...")
        output_dir = 'elasticnet_ablation_results'
        ...
        # 打印最终结果
        print("\n" + "="*80)
        print("ELASTICNET ABLATION STUDY RESULTS SUMMARY")
        print("="*80)
        for model_name, result in ablation_results.items():
