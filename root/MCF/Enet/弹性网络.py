...
def plot_applicability_domain(model, X_train, X_test, y_train, y_test):
    ...
    # -------------------------- 图例样式（独立设置）--------------------------
    ax.legend(
        fontsize=12,          # 图例字体大小（单独设置，不影响其他文本）
        ...
        prop={'weight': 'bold','size': 11,}  # 图例文字加粗（仅作用于图例）
    )
    ax.grid(True, linestyle='--', alpha=0.3)
    ...
    ax.text(
        ...
        fontsize=11,         # 统计文本字体大小（独立设置，比如改为10号）
        fontweight='bold', # 统计文本字重（独立设置：normal/bold/light，这里示例为普通）
        fontfamily='Arial',  # 统计文本字体（独立设置，可选）
        color='darkred',     # 统计文本颜色（独立设置）
        verticalalignment='top',  # 垂直对齐方式
        bbox=dict(
            boxstyle='round,pad=0.3',  # 文本框样式
            facecolor='lightyellow',   # 文本框背景色（独立设置）
            alpha=0.8,                 # 文本框透明度
            edgecolor='orange',        # 文本框边框色（独立设置）
