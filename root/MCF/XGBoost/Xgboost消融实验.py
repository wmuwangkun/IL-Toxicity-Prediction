...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("xgboost_ablation.log", encoding='utf-8'),
...
def train_and_tune_xgb(X_train, X_test, y_train, y_test, feature_subset, subset_name):
    """训练和调优XGBoost模型（保留原有参数调优功能）"""
    ...
    default_xgb.fit(X_train_subset, y_train)
    y_pred_default = default_xgb.predict(X_test_subset)
    default_metrics = {
...
def plot_ablation_radar_chart(ablation_results, output_dir):
    ...
    # 设置角度和标签
    ax.set_thetagrids(np.degrees(angles[:-1]), metrics, fontsize=10)  # 移除fontweight
    ...
    # 在右上角添加模型标识（与参考图片一致）
    ax.text(0.95, 0.95, 'XGBoost Model', transform=ax.transAxes, fontsize=14, 
...
def main():
    ...
    try:
        ...
        for subset_name, feature_subset in feature_subsets.items():
            result = train_and_tune_xgb(X_train, X_test, y_train, y_test, feature_subset, subset_name)
            ablation_results[subset_name] = result
        
        # 5. 绘制雷达图
        output_dir = './xgboost_ablation_results'
        ...
        logger.info(f"\n总执行时间: {elapsed:.2f}秒")
        logger.info("XGBoost消融实验完成!")
        
        # 打印最终结果
        print("\n" + "="*80)
        print("XGBOOST ABLATION STUDY RESULTS SUMMARY")
        print("="*80)
        for model_name, result in ablation_results.items():
            default_metrics = result['default_metrics']
            best_metrics = result['best_metrics']
