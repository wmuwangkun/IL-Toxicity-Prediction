...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("mlp_ablation.log", encoding='utf-8'),
...
def optimize_mlp_model(X_train, y_train, feature_cols):
    """优化MLP模型（简化参数搜索，去掉交叉验证）"""
    # MLP参数优化
    mlp_pipe = Pipeline([
    ...
    # 生成所有可能的参数组合
    param_combinations = []
    for hidden in mlp_params['mlp__hidden_layer_sizes']:
        for activation in mlp_params['mlp__activation']:
            for alpha in mlp_params['mlp__alpha']:
                for lr in mlp_params['mlp__learning_rate_init']:
                    for batch in mlp_params['mlp__batch_size']:
                        for max_iter in mlp_params['mlp__max_iter']:
                            param_combinations.append({
                                'mlp__hidden_layer_sizes': hidden,
                                'mlp__activation': activation,
                                'mlp__alpha': alpha,
                                'mlp__learning_rate_init': lr,
                                'mlp__batch_size': batch,
                                'mlp__max_iter': max_iter
                            })
    
    # 随机选择20个参数组合进行测试
    test_combinations = random.sample(param_combinations, min(20, len(param_combinations)))
    
    for i, params in enumerate(test_combinations):
        logger.info(f"测试参数组合 {i+1}/{len(test_combinations)}: {params}")
        
        # 设置参数
        mlp_pipe.set_params(**params)
        ...
        if score > best_score:
            best_score = score
            best_params = params
            best_estimator = mlp_pipe
    
    logger.info(f"MLP最佳参数: {best_params}")
    logger.info(f"MLP最佳训练集R2: {best_score:.4f}")
...
def train_mlp_model(X_train, X_test, y_train, y_test, feature_subset, subset_name):
    """训练MLP模型"""
    ...
    # 优化MLP模型
    mlp_model, best_params = optimize_mlp_model(X_train_subset, y_train, feature_subset)
    
    # 评估模型
    y_pred = mlp_model.predict(X_test_subset)
...
def plot_ablation_radar_chart(ablation_results, output_dir):
    ...
    # 添加标题 - 字体调小
    plt.title('MLP消融实验性能比较', fontsize=10, fontweight='bold', pad=20)  # 字体从12改为10
...
def main():
    try:
        ...
        for subset_name, subset_features in feature_subsets.items():
            result = train_mlp_model(X_train, X_test, y_train, y_test, subset_features, subset_name)
            ablation_results[subset_name] = result
        
        # 5. 绘制雷达图
        logger.info("绘制消融实验雷达图...")
        output_dir = 'mlp_ablation_results'
