...
def plot_results(results, X_train, X_test, y_train, y_test, rf_model):
    """绘制结果图表 - SCI风格"""
    ...
    ax.set_title('RF', fontsize=8, fontweight='bold')
    ...
    plt.savefig('rf_results/rf_scatter_plot.png', dpi=300, bbox_inches='tight')
    plt.savefig('rf_results/rf_scatter_plot.pdf', bbox_inches='tight')
    plt.close()
...
def plot_applicability_domain(X_train, X_test, y_train, y_test, rf_model):
    """绘制适用域分析图 - 修改为与参考代码一致的风格"""
    os.makedirs('rf_results', exist_ok=True)
...
def save_scatter_data(X_train, X_test, y_train, y_test, rf_model):
    """保存散点图数据到Excel文件"""
    os.makedirs('rf_results', exist_ok=True)
...
def save_results_to_excel(results, feature_cols, X_train, y_train, rf_model):
    """保存结果到Excel文件"""
    os.makedirs('rf_results', exist_ok=True)
    ...
    with pd.ExcelWriter('rf_results/random_forest_results.xlsx') as writer:
        ...
        # 2. 模型参数
        params_df = pd.DataFrame([results['model_params']])
...
if __name__ == "__main__":
    try:
        ...
        # 使用默认参数训练模型
        results, rf_model = train_rf_model(X_train, X_test, y_train, y_test)

        # 绘制残差直方图（测试集）
        test_residuals = (y_test - rf_model.predict(X_test)).to_numpy()
        plot_residual_histogram(test_residuals, 'rf_test', 'rf_results')
        
        # 绘制结果
        plot_results(results, X_train, X_test, y_train, y_test, rf_model)
        plot_applicability_domain(X_train, X_test, y_train, y_test, rf_model)
        
        # 保存散点图数据到Excel
        save_scatter_data(X_train, X_test, y_train, y_test, rf_model)
        
        # 保存结果到Excel
        save_results_to_excel(results, feature_cols, X_train, y_train, rf_model)
        
        # 保存模型
        joblib.dump(rf_model, 'rf_results/rf_model.pkl')
