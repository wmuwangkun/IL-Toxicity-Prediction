...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("xgboost_tuning.log"),
...
def train_and_tune_xgb(X_train, X_test, y_train, y_test):
    """训练和调优XGBoost模型"""
    # 默认模型
    ...
    # 训练集和测试集预测
    y_train_pred_default = default_xgb.predict(X_train)
    y_test_pred_default = default_xgb.predict(X_test)
    ...
    # 记录默认参数
    default_params = default_xgb.get_params()
    ...
    logger.info(f"总共测试 {len(param_combinations)} 种参数组合")
    ...
    for i, params in enumerate(param_combinations):
        ...
        try:
            ...
            if (i + 1) % 10 == 0:
                logger.info(f"已完成 {i + 1}/{len(param_combinations)} 种组合，当前最佳R²: {best_score:.4f}")
                
        except Exception as e:
            logger.warning(f"参数组合 {param_dict} 训练失败: {str(e)}")
            continue
    
    logger.info(f"参数搜索完成，耗时: {time.time() - start_time:.2f}秒")
    ...
    return results, default_xgb, best_model

def plot_results(results, X_train, X_test, y_train, y_test):
    """绘制预测结果图 - SCI风格，包含默认参数和调优后模型的散点图"""
    ...
    # 重新训练最佳模型
    best_xgb = XGBRegressor(** results['best_params'], random_state=42)
    ...
    ax1.set_title('Default Parameters - XGBoost', fontsize=8, fontweight='bold')
    ...
    ax2.set_title('XGBoost', fontsize=8, fontweight='bold')  # 修改标题为XGBoost
    ...
    # 保存对比图片
    plt.savefig('xgb_results/xgboost_comparison_scatter_plot.png', dpi=300, bbox_inches='tight')
    plt.savefig('xgb_results/xgboost_comparison_scatter_plot.pdf', bbox_inches='tight')
    plt.close()
    ...
    ax.set_title('Default XGBoost', fontsize=8, fontweight='bold')
    ...
    ax.text(0.95, 0.05, f'Training R² = {train_r2_default:.3f}\nTest R² = {test_r2_default:.3f}', 
             ...
             bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.9, 
                      edgecolor='gray', linewidth=0.5))
    plt.savefig('xgb_results/xgboost_default_scatter_plot.png', dpi=300, bbox_inches='tight')
    plt.savefig('xgb_results/xgboost_default_scatter_plot.pdf', bbox_inches='tight')
    plt.close()
    ...
    ax.set_xlabel('Experimental values', fontsize=7, fontweight='bold')
    ax.set_ylabel('Predicted values', fontsize=7, fontweight='bold')
    ax.set_title('XGBoost', fontsize=8, fontweight='bold')  # 修改标题为XGBoost
    ax.grid(True, linestyle='--', alpha=0.3)
    ...
    ax.text(0.95, 0.05, f'Training R² = {train_r2_best:.3f}\nTest R² = {test_r2_best:.3f}', 
             ...
             bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.9, 
                      edgecolor='gray', linewidth=0.5))
    plt.savefig('xgb_results/xgboost_tuned_scatter_plot.png', dpi=300, bbox_inches='tight')
    plt.savefig('xgb_results/xgboost_tuned_scatter_plot.pdf', bbox_inches='tight')
    plt.close()
    
    logger.info("散点图已保存至 xgb_results/ 目录")
...
def save_scatter_data(results, X_train, X_test, y_train, y_test):
    ...
    # 保存到Excel
    with pd.ExcelWriter('xgb_results/scatter_plot_data.xlsx') as writer:
    ...
    logger.info("散点图数据已保存至 xgb_results/scatter_plot_data.xlsx")
...
def plot_applicability_domain(model, X_train, X_test, y_train, y_test):
    ...
    plt.savefig('xgb_results/xgboost_applicability_domain.png', dpi=300, bbox_inches='tight')
    plt.savefig('xgb_results/xgboost_applicability_domain.tif', dpi=300, bbox_inches='tight', format='tiff')
    plt.close()
    ...
    with pd.ExcelWriter('xgb_results/xgboost_applicability_domain_data.xlsx') as writer:
        ad_df.to_excel(writer, sheet_name='All_Samples', index=False)
        summary_df.to_excel(writer, sheet_name='Summary', index=False)

    logger.info("XGBoost适用域分析已保存至 xgb_results/ 目录")
...
def save_results_to_excel(results, feature_cols, X_train, y_train):
    ...
    # 创建Excel写入器
    with pd.ExcelWriter('xgb_results/xgboost_results.xlsx') as writer:
        # 1. 模型性能比较 - 包含训练集和测试集
        # 创建性能比较表
        performance_data = []
...
def create_shap_analysis(X_train, X_test, feature_cols, model, model_name):
    """创建SHAP分析图表 - 完全按照参考代码的风格和样式"""
    import os
    import numpy as np
    import pandas as pd
    import matplotlib.pyplot as plt
    ...
    for label in ax.get_xticklabels():
        ...
        label.set_fontweight('extra bold')  # 更粗的字体
    ...
    # 保存图(a)
    base_path = f'xgb_results/{model_name}_global_explanation'
    ...
    # 保存图(b)
    base_path = f'xgb_results/{model_name}_local_explanation'
    plt.savefig(f'{base_path}.png', dpi=300, bbox_inches='tight', facecolor='white')
    plt.savefig(f'{base_path}.pdf', bbox_inches='tight', facecolor='white')
    plt.savefig(f'{base_path}.tif', dpi=300, bbox_inches='tight', facecolor='white', format='tiff')
    plt.close()
    ...
    with pd.ExcelWriter('xgb_results/feature_importance_analysis.xlsx') as writer:
    ...
    logger.info(f"特征重要性分析已保存至 xgb_results/feature_importance_analysis.xlsx")
...
if __name__ == "__main__":
    try:
        ...
        # 训练和调优模型
        results, default_xgb, best_xgb = train_and_tune_xgb(X_train, X_test, y_train, y_test)

        # 绘制残差直方图（仅调优模型测试集）
        tuned_test_residuals = (y_test - best_xgb.predict(X_test)).to_numpy()
        plot_residual_histogram(tuned_test_residuals, 'xgboost_tuned_test', 'xgb_results')
        ...
        # 绘制适用域分析
        plot_applicability_domain(best_xgb, X_train, X_test, y_train, y_test)
        
        # 生成SHAP分析（使用调优后的模型）
        create_shap_analysis(X_train, X_test, feature_cols, best_xgb, "xgboost_tuned")
        
        # 保存结果到Excel
        save_results_to_excel(results, feature_cols, X_train, y_train)
        
        # 保存模型
        joblib.dump(default_xgb, 'xgb_results/default_xgb_model.pkl')
        joblib.dump(best_xgb, 'xgb_results/tuned_xgb_model.pkl')
        
        # 记录执行时间
        elapsed = time.time() - start_time
        logger.info(f"总执行时间: {elapsed:.2f}秒")
        logger.info("XGBoost模型调优完成!")
        
        # 打印最终结果摘要
        print("\n" + "="*80)
        print("XGBOOST TUNING RESULTS SUMMARY")
        print("="*80)
